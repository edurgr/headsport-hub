...
export default function ContentPage() {
  ...
  // Removed base64 conversion; thumbnails should be uploaded to storage and referenced by path
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      if (thumbFile) {
        ...
        const bucket = process.env.NEXT_PUBLIC_UPLOADS_BUCKET || 'user-uploads';
        const key = `thumbnails/${Date.now()}-${Math.random().toString(36).slice(2)}-${editing.id}.jpg`;
        const { data: uploadRes, error: uploadErr } = await supabaseClient.storage
          .from(String(bucket))
          .upload(key, thumbFile, { upsert: true, contentType: thumbFile.type || 'image/jpeg' });
        if (!uploadErr) {
          thumbnail_path = uploadRes?.path || key;
        }
      }
      const tags = editTags
        .split(',')

    ...
    <div className="p-6">
      <div className="mb-8">
        ...
        <p className="mt-2 text-gray-600">
          Manage your athlete profile and content
        </p>
      </div>
...
function ContentSummary({ userId }: { userId: string }) {
  const [items, setItems] = useState<{ id: string; filename: string; created_at: string; thumbnail_url?: string }[]>([]);
  useEffect(() => {
    if (!userId) return;
    fetch(`/api/content/gallery?limit=5&user_id=${encodeURIComponent(userId)}`, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : Promise.reject(r))
      .then((j) => setItems((j.items || []).map((x: any) => ({ id: x.id, filename: x.filename, created_at: x.created_at, thumbnail_url: x.thumbnail_url }))))
      .catch(() => setItems([]));
  }, [userId]);
    ...
    <div className="grid grid-cols-5 gap-3">
      {items.map((i) => (
        <div key={i.id} className="text-center">
          ...
          <div className="mt-1 text-[11px] text-gray-700 truncate" title={i.filename}>{i.filename}</div>
          <div className="text-[10px] text-gray-400">{new Date(i.created_at).toLocaleDateString()}</div>
        </div>
      ))}
    </div>
  );
...
function OrdersSummary({ email }: { email: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/orders?scope=mine&athleteEmail=${encodeURIComponent(email)}`, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : Promise.reject(r))
      .then((j) => setOrders((j.orders || []).slice(0, 5).map((o: any) => ({ id: o.id, created_at: o.created_at, status: o.status }))))
      .catch(() => setOrders([]));
  }, [email]);
    ...
    <ul className="text-sm text-gray-700 space-y-1">
      {orders.map((o) => (
        <li key={o.id} className="flex items-center gap-2">
          <span className={`px-2 py-0.5 text-[10px] rounded-full ${o.status === 'approved' ? 'badge-success' : o.status === 'pending_approval' ? 'badge-warning' : 'badge-error'}`}>{o.status}</span>
...
function TeamSnapshot() {
  const [totals, setTotals] = useState<{ athletes: number; pending: number; uploads: number }>({ athletes: 0, pending: 0, uploads: 0 });
  const [recent, setRecent] = useState<{ approvals: { id: string; created_at: string; athlete: string }[]; pending: { id: string; created_at: string; athlete: string }[]}>({ approvals: [], pending: [] });
  ...
  useEffect(() => {
    ...
    fetch('/api/content/gallery?limit=20', { cache: 'no-store' })
      ...
      .then((j) => {
        const now = Date.now();
        const uploads24h = (j.items || []).filter((x: any) => now - Date.parse(x.created_at) < 24*3600*1000).length;
    ...
    fetch('/api/orders?scope=approved', { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : Promise.reject(r))
      .then((j) => setRecent((s) => ({ ...s, approvals: (j.orders || []).slice(0,5).map((o: any) => ({ id: o.id, created_at: o.approved_at || o.created_at, athlete: o.athlete_name || o.athlete_email })) })))
      .catch(() => {});
    fetch('/api/orders?scope=pending', { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : Promise.reject(r))
      .then((j) => setRecent((s) => ({ ...s, pending: (j.orders || []).slice(0,5).map((o: any) => ({ id: o.id, created_at: o.created_at, athlete: o.athlete_name || o.athlete_email })) })))
      .catch(() => {});
  }, []);
          ...
          {recent.approvals.length === 0 ? (
            ...
            <ul className="text-sm text-gray-700 space-y-1">
              {recent.approvals.map((a) => (
                <li key={a.id}>• {a.athlete} — {new Date(a.created_at).toLocaleDateString()}</li>
              ))}
            </ul>
          )}
        </div>
          ...
          {recent.pending.length === 0 ? (
            ...
            <ul className="text-sm text-gray-700 space-y-1">
              {recent.pending.map((p) => (
                <li key={p.id}>• {p.athlete} — {new Date(p.created_at).toLocaleDateString()}</li>
              ))}
            </ul>
          )}
        </div>

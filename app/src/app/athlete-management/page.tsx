...
export default function AthleteManagementPage() {
  ...
  const fetchAthletes = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/profiles/list?${params}`, { cache: 'no-store', redirect: 'follow' });
  ...
  return (
    <ProtectedRoute>
      <div className="p-6">
        {/* Page Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-[hsl(var(--foreground))] mb-2">Athlete Management</h1>
          ...
          {!process.env.NEXT_PUBLIC_SUPABASE_URL && (
            <div className="mt-4 p-4 rounded-lg" style={{ backgroundColor: 'hsl(var(--secondary))', border: '1px solid hsl(var(--border))' }}>
              <div className="flex items-center">
                <svg className="w-5 h-5 mr-2" style={{ color: 'hsl(var(--info))' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <div>
                  <p className="text-sm font-medium" style={{ color: 'hsl(var(--foreground))' }}>Demo Mode Active</p>
        ...
        <div className="grid md:grid-cols-4 gap-6 mb-8">
          <div className="p-4 rounded-lg border" style={{ backgroundColor: 'hsl(var(--secondary))', borderColor: 'hsl(var(--border))' }}>
            <div className="flex items-center justify-between">
              <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ backgroundColor: 'hsl(var(--border))' }}>
                <svg className="w-5 h-5" style={{ color: 'hsl(var(--foreground))' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                </svg>
              </div>
            </div>
            <div className="text-2xl font-bold text-[hsl(var(--foreground))] mt-2">{totalAthletes}</div>
          ...
          <div className="p-4 rounded-lg border" style={{ backgroundColor: 'hsl(var(--success) / 0.08)', borderColor: 'hsl(var(--success) / 0.3)' }}>
            ...
            <div className="text-sm" style={{ color: 'hsl(var(--success))' }}>Active Athletes</div>
            <div className="text-xs" style={{ color: 'hsl(var(--success))' }}>Competition ready</div>
          </div>
          ...
          <div className="p-4 rounded-lg border" style={{ backgroundColor: 'hsl(var(--warning) / 0.1)', borderColor: 'hsl(var(--warning) / 0.3)' }}>
            ...
            <div className="text-xs" style={{ color: 'hsl(var(--warning))' }}>Team leaders</div>
          </div>
          ...
          {loading ? (
            <div className="text-center py-8">
              <div className="inline-flex items-center px-4 py-2 font-semibold leading-6 text-[hsl(var(--muted))]">
                <svg className="animate-spin -ml-1 mr-3 h-5 w-" style={{ color: 'hsl(var(--muted))' }} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            ...
            <div className="text-center py-12">
              <svg className="mx-auto h-12 w-12 text-[hsl(var(--muted))]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {viewMode === 'cards' ? (
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredAthletes.map((athlete) => (
                    <div key={athlete.id} className="p-6 rounded-lg border hover:shadow-md transition-shadow" style={{ backgroundColor: 'hsl(var(--secondary))', borderColor: 'hsl(var(--border))' }}>
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center space-x-3">
                          <div className="w-12 h-12 rounded-full flex items-center justify-center" style={{ backgroundColor: 'hsl(var(--border))' }}>
                            <span className="text-sm font-medium text-[hsl(var(--foreground))]">
                              {athlete.name ? athlete.name.split(' ').map((n: string) => n[0]).join('') : 'A'}
                            </span>
                          </div>
                          <div>
                            <h3 className="text-lg font-semibold text-[hsl(var(--foreground))]">{athlete.name || 'Unnamed User'}</h3>
                      ...
                      <div className="flex items-center text-sm text-[hsl(var(--muted))]">
                        ...
                        Joined {athlete.created_at ? new Date(athlete.created_at).toLocaleDateString() : 'Unknown'}
                      </div>

                      {/* Stats */}
                      <div className="grid grid-cols-2 gap-2 pt-2" style={{ borderTop: '1px solid hsl(var(--border))' }}>
                        <div className="text-center">
                          <div className="text-lg font-semibold" style={{ color: 'hsl(var(--info))' }}>{athlete.totalOrders || 0}</div>
                          <div className="text-xs text-[hsl(var(--muted))]">Orders</div>
                        </div>
                        <div className="text-center">
                          <div className="text-lg font-semibold" style={{ color: 'hsl(var(--success))' }}>{athlete.equipmentCount || 0}</div>
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y" style={{ borderColor: 'hsl(var(--border))' }}>
                    <thead style={{ backgroundColor: 'hsl(var(--secondary))' }}>
                      <tr>
                        <th className="px-6 py-3 text-left text-xs font-medium text-[hsl(var(--muted))] uppercase tracking-wider">User</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-[hsl(var(--muted))] uppercase tracking-wider">Role</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-[hsl(var(--muted))] uppercase tracking-wider">Organization</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-[hsl(var(--muted))] uppercase tracking-wider">Contact</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-[hsl(var(--muted))] uppercase tracking-wider">Location</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-[hsl(var(--muted))] uppercase tracking-wider">Orders</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-[hsl(var(--muted))] uppercase tracking-wider">Equipment</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-[hsl(var(--muted))] uppercase tracking-wider">Last Order</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-[hsl(var(--muted))] uppercase tracking-wider">Joined</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-[hsl(var(--muted))] uppercase tracking-wider">Actions</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y" style={{ backgroundColor: 'hsl(var(--secondary))', borderColor: 'hsl(var(--border))' }}>
                      {filteredAthletes.map((athlete) => (
                        <tr key={athlete.id} className="hover:opacity-95">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-[hsl(var(--foreground))]">
                            {athlete.city && athlete.country ? `${athlete.city}, ${athlete.country}` : '-'}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-[hsl(var(--foreground))]">
                            <span className="font-medium" style={{ color: 'hsl(var(--info))' }}>{athlete.totalOrders || 0}</span>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-[hsl(var(--foreground))]">
                            <span className="font-medium" style={{ color: 'hsl(var(--success))' }}>{athlete.equipmentCount || 0}</span>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-[hsl(var(--foreground))]">
                            {athlete.lastOrderDate ? new Date(athlete.lastOrderDate).toLocaleDateString() : '-'}
                          </td>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div className="flex space-x-2">
                              ...
                              <button className="hover:opacity-80" style={{ color: 'hsl(var(--muted))' }}>Edit</button>
                            </div>
                          </td>
                        </tr>
                      ))}

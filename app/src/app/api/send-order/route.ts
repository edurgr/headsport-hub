...
const ProductSchema = z.object({
  ...
  name: z.string(),
  sku: z.string(),
  vertical_number: z.string(),
  category: z.string().optional(),
});

export async function POST(req: NextRequest) {
  // 1. Authentication and Authorization
  // Assuming you have a middleware or similar to check for authentication
  // For now, we'll just check if the user is authenticated
  const user = req.headers.get('Authorization'); // Example: check for a token
  if (!user) {
  ...
  const sanitizedAddress = body.shippingAddress; // Already sanitized by Zod

  // Basic validation
  console.log('🔍 Validating rows:', Array.isArray(body.rows), body.rows?.length);
  if (!Array.isArray(body.rows) || body.rows.length === 0) {
    console.log('❌ Empty order validation failed');
    return addSecurityHeaders(createSecureErrorResponse('Empty order', 400));
  }

  console.log('🔍 Validating athlete email:', body.athleteEmail);
  if (!body.athleteEmail) {
  ...
  console.log('🔍 Validating shipping address:', sanitizedAddress);
  if (!sanitizedAddress) {
  ...
  const emailValidation = validateRequest(schemas.email, body.athleteEmail);
  ...
  const { data: order, error: orderError } = await sb
    ...
    .insert({
      ...
      athlete_email: body.athleteEmail,
      athlete_name: (athlete && athlete.name) || body.athleteEmail.split('@')[0],
      status: 'pending_approval', // Changed from 'pending' to 'pending_approval'
      shipping_address: sanitizedAddress,
      notes: `Order with ${body.rows.length} items - Pending manager approval`,
  ...
  const orderItems = body.rows.map((row: any) => ({
    order_id: order.id,
  ...
  const enrichedRows = body.rows.map((row: any) => ({
    product: {

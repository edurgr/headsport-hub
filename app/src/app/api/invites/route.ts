...
export async function POST(req: NextRequest) {
  const { email, rolePreset = 'athlete', message, expiresInDays = Number(process.env.INVITE_TOKEN_TTL_DAYS || 7) } = await req.json();

  const sb = await supabaseServer();
  const token = crypto.randomUUID();
  const expires_at = new Date(Date.now() + expiresInDays * 24 * 3600 * 1000).toISOString();

  const { error } = await sb.from('invites').insert({ email, role_preset: rolePreset, token, expires_at, used: false });

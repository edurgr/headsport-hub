...
export async function sendEmail(to: string, subject: string, html: string, text?: string): Promise<boolean> {
  ...
  if (process.env.AWS_SES_ACCESS_KEY_ID && process.env.AWS_SES_SECRET_ACCESS_KEY && process.env.AWS_SES_REGION) {
    try {
      ...
      const sesResponse = await fetch(`https://email.${process.env.AWS_SES_REGION}.amazonaws.com/`, {
        method: 'POST',
        headers: {
...
export async function sendInvitationEmail(email: string, rolePreset: string, inviteUrl: string, message?: string): Promise<boolean> {
...
export async function sendOrderConfirmationEmail(email: string, orderId: string, items: Array<{
  product: {
    name: string;
    sku: string;
    category?: string;

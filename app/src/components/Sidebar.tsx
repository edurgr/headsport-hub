...
export default function Sidebar() {
  ...
  return (
    <div className={`bg-[hsl(var(--sidebar))] text-[hsl(var(--sidebar-contrast))] transition-all duration-300 ${isCollapsed ? 'w-12 sm:w-16' : 'w-64'}`}>
      <div className="flex flex-col h-full">
        <div className="px-4 py-6">
          <div className="flex items-center gap-3">
            <div className="relative w-10 h-10 rounded-md overflow-hidden bg-[hsl(var(--sidebar))] flex items-center justify-center" style={{ border: '1px solid hsl(var(--sidebar-contrast) / 0.12)' }}>
              <Image src={logoSrc} width={36} height={36} alt={brandName} style={{ width: 'auto', height: 'auto' }} />
            </div>
            {!isCollapsed && (
              <div className="min-w-0">
                <h1 className="text-base font-semibold tracking-tight">{brandName}</h1>
                <p className="text-xs text-[hsl(var(--sidebar-contrast) / 0.7)]">Sports Management Platform</p>
              </div>
            )}
          </div>
          {!isCollapsed && (
            <div className="mt-3">
              <span className="inline-flex px-2 py-1 text-[10px] font-semibold rounded-full" style={{ backgroundColor: 'hsl(var(--sidebar-contrast) / 0.12)', color: 'hsl(var(--sidebar-contrast))' }}>
          ...
          {isCollapsed ? (
            <div className="space-y-2">
              <div className="flex items-center justify-center">
                <Link href="/profile" className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-white/5" title="Profile" aria-label="Profile">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <Link href="/profile" className="flex items-center space-x-3 flex-1 min-w-0 rounded-lg p-2 hover:bg-white/5" style={{ border: '1px solid hsl(var(--sidebar-contrast) / 0.12)' }}>
                  <div className="w-8 h-8 rounded-full flex items-center justify-center" style={{ backgroundColor: 'hsl(var(--sidebar-contrast) / 0.12)' }}>
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                    </svg>
                  </div>

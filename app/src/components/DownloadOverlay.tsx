...
export default function DownloadOverlay() {
  ...
  return (
    <div className="fixed inset-0 z-[70] pointer-events-none">
      ...
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-auto w-[90%] max-w-md rounded-xl shadow-lg p-5" style={{ backgroundColor: 'hsl(var(--secondary))', border: '1px solid hsl(var(--border))' }}>
        <div className="mb-3 text-center font-medium" style={{ color: 'hsl(var(--foreground))' }}>{label || 'Downloading…'}</div>
        ...
        <div className="mt-2 text-center text-xs" style={{ color: 'hsl(var(--muted))' }}>{Math.round(progressPct)}%</div>
      </div>
    </div>
  );
}
